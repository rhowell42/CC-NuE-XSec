...
def MakeSurface(histogram,outdir,deltam=1,U_e4=0,U_tau4=0,makePlot=False,exclude="",lam=1):
...
if __name__ == "__main__":
    ...
    U_e4s[0] = 0
    U_e4 = U_e4s[AnalysisConfig.U_e4]

    if not AnalysisConfig.grid: # surface plot
        m_toloop = np.logspace(-1,2,100)
        for m in m_toloop:
            MakeSurface(sample_histogram,AnalysisConfig.output_dir,m,AnalysisConfig.U_e4,AnalysisConfig.U_tau4,lam=AnalysisConfig.lambdaValue,exclude=AnalysisConfig.exclude)
    else:
        MakeSurface(sample_histogram,AnalysisConfig.output_dir,AnalysisConfig.delta_m,AnalysisConfig.U_e4,AnalysisConfig.U_tau4,makePlot=False,lam=AnalysisConfig.lambdaValue,exclude=AnalysisConfig.exclude)
