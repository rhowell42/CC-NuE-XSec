...
def submitJob(tupleName,dm2):
  ...
  my_wrapper.write( "py3env/bin/python3 makeSurface.py --pseudodata --grid --delta_m %f --output $CONDOR_DIR_HISTS %s &> $CONDOR_DIR_LOGS/%s-${PROCESS}.log\n" % (dm2,argstring,tupleName) )
...
#    sys.exit(0)
  ...
  njobs = 1
  if os.path.exists("grid_surface.sh"):
    os.system( "rm grid_surface.sh")

  for m in range(1,201):
    cmdString = "FitSpace_{}".format(m)
    submitJob(cmdString,m)
