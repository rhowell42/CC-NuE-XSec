...
def submitJob(tupleName,dm2,ue4,umu4):
  ...
  my_wrapper.write( "py3env/bin/python3 testFitter.py --pseudodata --grid --delta_m %f --U_e4 %f --U_mu4 %f --output $CONDOR_DIR_HISTS %s &> $CONDOR_DIR_LOGS/%s-${PROCESS}.log\n" % (dm2,ue4,umu4,argstring,tupleName) )
...
#    sys.exit(0)
  ...
  processingID = '%s_%s-%s' % ("Oscillation_Fitter", dt.date.today() , dt.datetime.today().strftime("%H%M%S") )
  outdir_hists = "/pnfs/minerva/scratch/users/%s/Oscillation_Fitter_Texts" % (os.environ["USER"])
  os.system( "mkdir -p %s" % outdir_hists )
  outdir_logs = "/pnfs/minerva/scratch/users/%s/Oscillation_Fitter_logs" % (os.environ["USER"])
  ...
  for m in [2,4,8,12,20,50,100]:
    for ue4 in [0,0.01,0.04,0.1]:
      for umu4 in [0,0.002,.013,.173,.410]:
        if ue4 == 0 and umu4 == 0:
          continue
        cmdString = "Fitter_{}_{}_{}".format(m,ue4,umu4)
        submitJob(cmdString,m,ue4,umu4)
